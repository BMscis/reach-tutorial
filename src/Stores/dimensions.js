...
export const MainContainer = writable({
  left: 0,
  center: 0,
  right: 0,
  direction: "columns",
...
const setupBottomContainer = ((value) => {
  ...
  switch ((value[0] / value[1]).toFixed(2) >= 1) {
    case true:
      ...
      blockWidth = fullBlockWidth
      blockHeight = blockSize(alterVal[1], 0.80)
      nftWidth = NFTCardSize(blockSize(blockWidth, 0.8), blockHeight)
      break;
    case false:
...
const SetMainContainer = ((value) => {
  let [width, height] = [value[0], value[1]]
  let left
  let right
  let center
  let [direction, size] = MainGridDisplay(width, height)
  global_direction = direction
  global_size = size
  console.log("size", size)
  let fullWidth = blockSize(width, 0.33)
  switch (direction) {
    case "columns":
      switch (size) {
        case 2:
          left = 0
          center = width - fullWidth
          right = fullWidth
          break;
        case 3:
          left = 300
          center = width - 300 - fullWidth
          right = fullWidth
      ...
      break;
    case "rows":
      let sizeHeight = (height - blockSize(value[1], 0.1)).toFixed(2)
      left = sizeHeight
      center = sizeHeight
      right = sizeHeight
      break;
...
const NFTCardSize = ((width, height) => {
  //ratio = if width is greater than height then divide height by width else divide width by height
  //if ratio is greater than 0.55 reduce width so that width divided by height is equal to 0.55
  let ratio = width / height
  ratio > 0.55 ? width = height * 0.55 : width = width
...
const MainGridDisplay = ((width, height) => {
  ...
  fittingScreens == 0 ? fittingScreens = 1 : fittingScreens = fittingScreens
  return fittingScreens > 1 ? ["columns", fittingScreens] : ["rows", 1]
})